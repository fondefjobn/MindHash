...
def merge_new_config(config, new_config, rel_path=''):
    if '_BASE_CONFIG_' in new_config:
        with open(rel_path + new_config['_BASE_CONFIG_'], 'r') as f:
            try:
    ...
    for key, val in new_config.items():
        ...
        if key not in config:
            config[key] = EasyDict()
        merge_new_config(config[key], val, rel_path=rel_path)
    return config


def cfg_from_yaml_file(cfg_file, config, rel_path=''):
    with open(cfg_file, 'r') as f:
        ...
        merge_new_config(config=config, new_config=new_config, rel_path=rel_path)
