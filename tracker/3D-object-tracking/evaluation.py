...
def evaluation(real_objects, hypothesis, n_frames):
    ...
    def count_in_frames(target):
        return np.array(
            [[1 if frame in entries else 0 for frame in range(n_frames)] for entries in target.values()]).sum(axis=0)
    ...
    for frame in range(n_frames):
        ...
        o_candidates = torch.Tensor(np.array(o_candidates))
        h_candidates = torch.Tensor(np.array(h_candidates))
        ...
        for o_idx, h_idx in frame_mapping.items():
            ...
            if n_h_l_frame[label] < c_l_frame[label]:
                print(f"Something went wrong: having {c_l_frame[label]} mapping entries for label {label_to_str[label]}"
                      f" but only {n_h_l_frame[label]} hypothesis")
                return

            if n_o_l_frame[label] < c_l_frame[label]:
                print(f"Something went wrong: having {c_l_frame[label]} mapping entries for label {label_to_str[label]}"
                      f" but only {n_o_l_frame[label]} true objects")
                return
    ...
    for label in labels:
        ...
        MOTA = 1 - (misses_sum + fp_sum + mme_sum) / g_sum
        print(
            f"{label_to_str[label]:10} MOTA: {MOTA} | c {c_sum} | g {g_sum} | misses {misses_sum} | fp {fp_sum} | mme {mme_sum}")
