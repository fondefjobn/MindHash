...
def main(args):
    ...
    print(f"Loading segment from path {args.segment}")

    dataset = tf.data.TFRecordDataset([args.segment])
    frames = []
    ...
    for i, frame in enumerate(frames):
        ...
        if args.dropout > 0:
            ...
            n_dropout = int(n_detections * args.dropout)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Run a tracking algorithm on a segment of the Waymo Open Dataset')

    parser.add_argument('-s', '--segment', type=str, required=True,
                        help='Path to a segment .tfrecord file.')
    parser.add_argument('-d', '--dropout', type=float, default=0, dest='dropout',
